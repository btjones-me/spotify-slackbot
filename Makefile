...
checktypes: .venv ## check types with mypy
	poetry run mypy --ignore-missing-imports spotify_slackbot tests

checkstyle: .venv ## check style with flake8, isort and black
	poetry run flake8 spotify_slackbot tests
	poetry run isort --check-only --profile black spotify_slackbot tests
	poetry run black --check --diff spotify_slackbot tests

fixstyle: .venv ## fix black and isort style violations
	poetry run isort --profile black spotify_slackbot tests
	poetry run black spotify_slackbot tests

sast: .venv ## run static application security testing
	poetry run bandit -r spotify_slackbot
...
coverage: ## check code coverage quickly with the default Python
	poetry run coverage run --source spotify_slackbot -m pytest
